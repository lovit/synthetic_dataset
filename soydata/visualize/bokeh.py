...
def scatterplot(
    x, y=None, labels=None, color="#5e4fa2", size=5, alpha=0.95, p=None, show_inline=True, marker="circle", **kargs
):
    ...
    if labels is not None:
        color = initialize_palette(labels=labels, palette=kargs.get("palette", None), n_labels=kargs.get("n_labels", -1))[
            : x.shape[0]
        ]
...
def lineplot(X, y=None, pairs=None, line_width=0.5, line_dash=(5, 3), line_color="#2b83ba", p=None, show_inline=True, **kargs):
    if p is None:
        p = initialize_figure(
            title=kargs.get("title", None),
            height=kargs.get("height", 600),

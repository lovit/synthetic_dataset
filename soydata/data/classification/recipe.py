...
def make_decision_tree_data_from_recipe(n_samples, recipe):
    ...
    for type, volume, label, x_min, x_max, y_min, y_max in recipe:
        ...
        if type == "rec":
            ...
            Xs = make_triangular(ns, upper=True, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max)
        elif type == "lower":
            Xs = make_triangular(ns, upper=False, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max)
        else:
